...
- **Tests and evals import canonical names from `agent/agents/__init__.py`** — `run_agent`, `AgentOutput`, `AgentDeps`, `agent` — never from a stub module directly. `scripts/choose_pattern.py` maintains the aliased import line in that file; if you edit it by hand, keep the four canonical names intact. `tests/test_stubs.py` smoke-tests each stub and auto-skips ones the script deleted. This canonical re-export is scoped by design to exactly one *primary* agent — an app needing several independent, differently-shaped agents adds them with `scripts/add_agent.py <name>` instead, which never touches `agent/agents/__init__.py`; each additional agent lives in its own module and is imported directly, not through the canonical export. See "Multiple agents" in the README.
